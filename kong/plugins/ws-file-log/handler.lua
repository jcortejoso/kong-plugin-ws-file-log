...
local WsFileLogHandler = {
  PRIORITY = 1000,
  VERSION = "0.1",
}

function WsFileLogHandler:ws_hadshake()
  ...
  local message = "New handshake from: " .. remote_addr
  log(conf, message)
end

function WsFileLogHandler:ws_client_frame()
  if conf.custom_fields_by_lua then
    local set_serialize_value = kong.log.set_serialize_value
    for key, expression in pairs(conf.custom_fields_by_lua) do
      set_serialize_value(key, sandbox(expression, sandbox_opts)())
  ...
  local remote_addr = ngx.var.remote_addr
  local frame = kong.websocket.get_frame()
  local message = "Websocker frame from client" .. remote_addr .. " - " .. frame
  log(conf, message)
end

return WsFileLogHandler
